...
output[0] = {"Ensemble_Elements": "RFC", "Meta_Classifier": "SVC", "Accuracy_Score_CV": acc_score_cv, "Accuracy_Score": acc_score, "Runtime": time_}
...
for j in range(0,9):
	...
	pipeline2 = Pipeline([
		#('rte', RandomTreesEmbedding(n_estimators=est,max_depth=depth,random_state=rand_int)),
		('lr', LogisticRegression(random_state=rand_int)),
	])
	models.append(pipeline2)

ensemble_cv.add(models)
ensemble.add(models)

# Attach the final meta estimator
ensemble_cv.add_meta(LogisticRegression())
...
output[1] = {"Ensemble_Elements": "LR", "Meta_Classifier": "LR", "Accuracy_Score_CV": acc_score_cv, "Accuracy_Score": acc_score, "Runtime": time_}
